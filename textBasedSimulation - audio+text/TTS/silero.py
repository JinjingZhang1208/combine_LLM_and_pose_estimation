...
def init():
    ...
    if settings.GetOption("tts_enabled") and tts is None:
        ...
        if tts is not None and not failed:
            ...
            return False
def audio_processing(inputAudio):
    Audio = inputAudio.numpy()
    outputAudio=Audio.tobytes()
    return outputAudio

def addToStream(stream,audio):
...
# audio, sample_rate = tts.tts(text)
# text1 = "Yes, I live close to Central Park! It's such a beautiful park, and I take my dog, Louie, for walks there every morning. It's our favorite activity!(Happy, Dance)"
# audio1, sample_rate = tts.tts(text1)
# # tts.play_audio(audio,9)
# print("finish tts")
# audio_data=audio
# import pyaudio
# import numpy as np
...
#                 output_device_index=8)
#
# # Assuming `audio_data` is a stereo NumPy array of floats
# try:
#     while(1):
#         print("start writing to stream")
#         audio_numpy = audio_data.numpy()
#         stream.write(audio_numpy.tobytes())
#         print("end writing to stream")
#         print("add 2nd sentence")
#         audio_numpy = audio1.numpy()
#         stream.write(audio_numpy.tobytes())
# except KeyboardInterrupt:
#     # Handle interrupt to stop the stream gracefully
